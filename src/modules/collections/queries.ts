...
function cleanSchema(input: unknown): PropertySchema {
...
export async function updateCollection(id: string, input: UpdateCollectionInput): Promise<CollectionDTO | null> {
  ...
  if (input.schema !== undefined) data.schema = json(cleanSchema(input.schema));
...
async function cleanArticleId(value: unknown, exceptItemId?: string): Promise<string | null | undefined> {
  if (value === undefined) return undefined;
  if (value === null || value === "") return null;
  if (typeof value !== "string") throw new CollectionValidationError("invalid page", { articleId: "must be an id" });
  const article = await prisma.article.findUnique({ where: { id: value }, select: { id: true } });
  if (!article) throw new CollectionValidationError("page not found", { articleId: "unknown page" });
  const linked = await prisma.collectionItem.findFirst({
...
function cleanProperties(schema: PropertySchema, input: unknown): PropertyValues {
...
export async function createItem(collection: CollectionDTO, input: ItemInput): Promise<ItemDTO> {
  ...
  const properties = cleanProperties(collection.schema, input.properties ?? {});
...
export async function updateItem(collection: CollectionDTO, itemId: string, input: ItemInput): Promise<ItemDTO | null> {
  const current = await prisma.collectionItem.findFirst({ where: { id: itemId, collectionId: collection.id } });
  if (!current) return null;
  ...
  if (input.properties !== undefined) {
    ...
    const patch = typeof input.properties === "object" && input.properties !== null ? input.properties : {};
    data.properties = json(cleanProperties(collection.schema, { ...stored, ...patch }));
  }
  const articleId = await cleanArticleId(input.articleId, itemId);
