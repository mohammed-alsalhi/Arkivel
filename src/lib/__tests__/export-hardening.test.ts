...
describe("export hardening", () => {
  it("covers the roadmap export formats and status lifecycle", () => {
    expect(EXPORT_FORMATS).toEqual(["markdown", "html", "json", "csv", "zip", "mediawiki", "database"]);
    expect(EXPORT_STATUSES).toEqual(["queued", "running", "completed", "failed", "cancelled", "retrying"]);
    expect(exportHardeningContract.progressStages).toEqual([
      "queued",
      "collecting",
      "rendering",
      "packaging",
