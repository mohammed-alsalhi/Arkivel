  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        className={clsx(
          "flex items-center gap-1 h-6 px-2 text-[11px] border border-border rounded transition-colors",
          "text-foreground hover:bg-surface-hover",
          open && "bg-surface-hover"
        )}
      >
        {loaded && initials ? (
          <span className="flex items-center justify-center w-4 h-4 rounded-full bg-heading text-surface text-[9px] font-bold leading-none">
            {initials}
          </span>
        ) : (
          <UserIcon />
