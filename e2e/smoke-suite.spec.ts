...
test.describe("seeded article paths", () => {
  ...
  test("reads the seeded article", async ({ page }) => {
    ...
    await expect(page.locator("#article-content")).toContainText("adr-001");
  ...
  test("opens the seeded article editor for the smoke admin", async ({ page }) => {
    ...
    expect(response?.status()).toBeLessThan(400);
    await expect(page.getByRole("heading", { level: 1, name: "Editing: architecture decisions" })).toBeVisible();
    await expect(page.getByLabel("Title:")).toHaveValue("architecture decisions");
