...
- **Admin customization:** `/admin/customization` is a tabbed env-first workbench for brand copy, logos, style presets, color themes, layout presets, feature flags, live preview panels, source badges, theme-pack validation, and copy-ready `.env`, `.env.local`, Vercel, or Docker Compose values; `/admin/marketplace` filters the built-in catalog and shows catalog health
