...
- **Customization manifest** — `/api/customization` exposes grouped brand, style preset, color theme, layout, feature flag, limit, map, reusable component, marketplace registry/import-preview, plugin, theme-pack, and theme-hook metadata for self-hosters, plugins, forks, and deployment dashboards.
...
- **Built-in Marketplace** — `/admin/marketplace` lists and filters styles, color themes, layouts, component packs, plugins, and theme packs with versioned local-registry health, schema/source metadata, license/checksum details, preview-only pack import parsing, theme token diffs, validation issue reporting, and no remote code fetching.
