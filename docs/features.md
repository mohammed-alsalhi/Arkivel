...
- **Customization Studio** — `/admin/customization` is a tabbed env-first workbench for brand copy, logos, style, color theme, layout, feature flags, browser-local drafts, named presets, active-vs-draft diffs, diagnostics, preview panels, source badges, theme-pack JSON validation, downloadable support reports, and copy-ready `.env`, `.env.local`, Vercel, or Docker Compose output.
