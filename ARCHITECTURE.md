...
`src/lib/site-mode.ts` resolves the server-only deployment surface. Wiki mode is the safe default and renders the database/auth-aware application shell. `ARKIVEL_SITE_MODE=product` renders the database-free Arkivel product shell, homepage, and `/docs` surface before any Prisma or session work runs. This lets one public Git repository feed multiple isolated Vercel projects without sharing database, storage, secret, branding, or domain configuration.

`src/proxy.ts` is the product deployment boundary. It returns 404 for wiki pages and API routes in product mode while allowing the homepage, docs, references, metadata, analytics, and static assets. Wiki mode keeps the full application surface.

Application builds only run `next build`. Production schema changes are a separate, reviewed operation after a verified backup; the current repository still uses `npx prisma db push` because its historical migration chain is incomplete. Builds must never mutate the schema or call `prisma db push --accept-data-loss`.

Reusable extension metadata lives in `src/lib/marketplace.ts`. Built-in style presets such as `classic-wiki`, `atlas-modern`, and `simplesque`, color themes such as `standard`, `forest`, and `ember`, layout presets, component packs, theme packs, and plugin manifests share the same versioned local registry contract: stable id, kind, version, compatibility, author, license, local source, status, screenshots, and checksums. Style and color selectors target any matching `data-*` scope so the live root and nested Customization Studio previews use the same tokens. The v1 plugin manifest contract lives in `src/lib/plugin-manifest.ts`; it declares identity, compatibility, permissions, routes, settings, widgets, hooks, jobs, storage, API scopes, webhooks, examples, validation issues, and Arkivel/plugin API compatibility metadata before any runtime loader is enabled. Preview-only pack import parsing lives in `src/lib/marketplace-import.ts` so future local installs can reuse the same schema and safety checks.
...
Audit trail helpers live in `src/lib/audit.ts`, with append-only `AuditLog` rows storing actor, target, workspace, severity, success, network metadata, and JSON details. `/api/admin/audit-log` supports actor/action/target/workspace/severity/success/date filters plus JSON export with summary, standard, strict, or full redaction. Alert trigger metadata and retention defaults are documented in `docs/audit-trail.md`.
...
Collaboration controls live in `src/lib/collaboration-controls.ts`. They define workspace-aware policy for co-authors, edit locks, review assignments, comments, mentions, notifications, workspace activity digests, and contribution summaries. Anonymous RSS, Atom, and sitemap outputs only include legacy unscoped published articles or public workspace articles; API v1 callers can read unscoped articles plus public, owned, or actively-membered workspace content.

Editorial governance contracts live in `src/lib/editorial-governance.ts`. Review requests carry due dates, required reviewer ids, approval thresholds, change-request cycle counts, and decision notes. Claim reviews carry evidence and expiration metadata for disputed/needs-source/stale/rejected/unreviewed queues. Verification stamps include reviewer, evidence, and expiration metadata, and `/api/admin/editorial-governance/summary` aggregates release blockers, editorial risk, claim queues, verification renewals, and owner gaps.
...
Component-pack developer tooling lives in `scripts/generate-component-pack.ts`, `scripts/validate-component-pack.ts`, `examples/marketplace/component-pack`, and `docs/component-pack-preview-harness.md`. The tooling scaffolds preview-safe packs, validates import-preview manifests and slot ids, and gives authors typed article, category, dashboard, marketplace, and editor fixture data before runtime pack loading exists.
...
- Built-in style presets, color themes, layout presets, layout composition hooks, component packs, theme pack schemas, marketplace registry metadata, validation summaries, and marketplace items.
...
`src/lib/external-references.ts` defines the v4.93.1 cross-instance reference contract for external articles, spaces, sources, and imported snapshots, with provenance labels, broken-reference diagnostics, and public-index planning that allows only explicit public-indexable references while omitting authenticated, private, and sensitive metadata.

Shared CSS guardrails in `src/app/globals.css` enforce touch-target minimums, overflow wrapping, dialog bounds, and phone-width horizontal overflow protection.

`e2e/smoke-suite.spec.ts` exercises the required product smoke routes, and `scripts/seed-smoke-fixtures.mjs` seeds repeatable database fixtures.

`scripts/verify-docs-sync.mjs` verifies package, changelog, roadmap, docs, in-app docs, and `/api/customization` alignment.
...
`src/lib/public-api-v1.ts` is the pre-v5 public API contract source for articles, categories, tags, revisions, search, customization, marketplace, plugins, webhooks, exports, and health. It defines stable endpoint metadata, pagination/filter/sort contracts, error shape, rate-limit headers, fixture responses, and OpenAPI generation. `GET /api/v1/contract` and `GET /api/v1/openapi.json` expose the same contract; update `docs/api-v1-migration.md` and in-app API docs when changing v1 behavior.

`src/lib/sdk-types.ts` layers SDK-ready type names, API-key scopes, generated client snippets, and sample script metadata on top of the frozen v1 contract. `GET /api/v1/sdk` exposes this metadata for generated clients and docs.
...
### Observability
`src/lib/observability.ts` defines structured log categories, metric types, privacy controls, metadata redaction, and event-feed contracts. `GET/POST /api/admin/observability` exposes an admin event feed and privacy controls for `/admin/observability`.
...
### Cache Strategy
`src/lib/cache-strategy.ts` defines invalidation rules for articles, categories, tags, feeds, sitemap, customization, marketplace metadata, plugin manifests, search, and dashboards. Article/category/tag write APIs call `invalidateCacheForEvent()` after successful creates.

`src/lib/security-review.ts` defines the v4.89.0 security review contract for auth, sessions, API keys, CSRF-sensitive writes, webhooks, imports, uploads, plugin manifests, marketplace packs, admin routes, and exports. `middleware.ts` applies conservative browser headers and a report-only CSP.
