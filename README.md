...
| Cache strategy | `docs/cache-strategy.md` | Invalidation rules, stale warnings, and CDN/Vercel/Docker/reverse-proxy recipes. |
| Security review | `docs/security-review.md` | Browser security headers, reviewed auth/API/plugin/export surfaces, abuse-case gates, supply-chain checklist, and pre-v5 threat-model draft. |
| Marketplace security | `docs/secure-marketplace-plugins.md` | Unsafe pack rejection, blocked permissions/hooks, dangerous plugin capability warnings, local-only install guidance, and provenance/checksum planning. |
| External references | `docs/external-references.md` | Cross-instance article, space, source, and snapshot metadata with provenance labels, diagnostics, and privacy-safe public index planning. |
| API documentation | `/api-docs` | In-app reference for endpoints, auth, and errors. |
| Customization manifest | `/api/customization` | Public brand, style preset, color theme, layout, layout composition, feature flag, limit, map, component pack, theme-pack, and marketplace registry manifest for self-hosters and plugins. |
...
| Search API contract | `docs/search-powered-plugins.md` | Stable typed search result shapes for plugins, widgets, dashboards, external tools, mobile clients, privacy metadata, retention policy, and webhook planning. |
| Editor reliability | `docs/editor-troubleshooting.md` | Collaborative sync, draft recovery, autosave repair, snapshot restore/compare/discard, editor health diagnostics, and large-document fixture metadata. |
| Editor controls | `docs/editor-controls.md` | Reusable editor primitives, plugin extension points, block templates, and shortcut scopes for self-hosters and trusted plugins. |
| Collaboration UX | `docs/collaboration-ux.md` | Live editing states, presence names, conflict/reconnect copy, inline review planning, notification routing, mobile QA, and accessibility checkpoints. |
...
| Upgrade assistant | `docs/v5-upgrade-planning.md` | v5 readiness checklist, pre-upgrade diagnostics, post-upgrade smoke checks, compatibility warnings, and release-note/migration doc links. |
| Documentation onboarding | `docs/index.md`, `docs/maintainer-guide.md` | Maintainer guide, setup paths, troubleshooting, docs IA review, and practical docs link tests. |
